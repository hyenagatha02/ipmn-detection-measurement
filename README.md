...
│   │   └── outputImgs/         # Intermediate outputs generated during processing
│   │
│   ├── nnUNet-master/          # Customized nnU-Net v2
│   ├── nnUnet_raw/
│   ├── nnUnet_preprocessed/
│   └── nnUnet_results/         # Directory for trained nnU-Net model weights
│
...
Intermediate outputs will be generated under

```
model/dataset/outputImgs/
```

Final measurement results will be saved under
...
Before running inference, place the trained nnU-Net checkpoint files under:
...
Compared with the official nnU-Net v2 implementation, only the preprocessing and inference pipeline were modified to support the proposed workflow. The core nnU-Net architecture, network design, and self-configuring training pipeline remain unchanged.
...
For additional details, see `model/README_nnUNet.md`.
