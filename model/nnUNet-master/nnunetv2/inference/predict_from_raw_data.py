...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            ...
            # [Error 2]
            # File "C:\Users\MI2RL\Desktop\01_duct_segmentation\ALL_AT_ONCE\model\nnUNet-master\nnunetv2\inference\predict_from_raw_data.py", line 376, in predict_from_data_iterator
            #     for preprocessed in data_iterator:
            #                         ^^^^^^^^^^^^^
            # File "C:\Users\MI2RL\Desktop\01_duct_segmentation\ALL_AT_ONCE\model\nnUNet-master\nnunetv2\inference\data_iterators.py", line 152, in preprocessing_iterator_fromfiles
            #     [i.pin_memory() for i in item.values() if isinstance(i, torch.Tensor)]
            #     ~~~~~~~~~~~~^^
            # RuntimeError: Cannot access accelerator device when none is available.
            # Process SpawnProcess-20:
            # Traceback (most recent call last):
            # File "C:\Users\MI2RL\AppData\Local\Programs\Python\Python313\Lib\multiprocessing\connection.py", line 328, in _recv_bytes
            ...
            # print(f'** torch version : {pytorch_version}')
            # print(f'** self.device   : {self.device}')
            
            if version.parse(pytorch_version) >= version.parse("2.6.0"):
                checkpoint = torch.load(join(model_training_output_dir, f'fold_{f}', checkpoint_name), 
...
def predict_entry_point():
    print("\n  ++++++ CUSTOMIZED (../nnUNet-master/nnunetv2/inference/predict_from_raw_data.py) ++++++")
    import argparse
    parser = argparse.ArgumentParser(description='Use this to run inference with nnU-Net. This function is used when '
                                                 'you want to manually specify a folder containing a trained nnU-Net '
                                                 'model. This is useful when the nnunet environment variables '
