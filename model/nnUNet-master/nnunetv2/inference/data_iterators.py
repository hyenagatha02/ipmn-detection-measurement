...
def preprocess_fromfiles_save_to_queue(list_of_lists: List[List[str]],
    ...
    try:
        ...
        # #### added by hyenagatha02 (added in default_preprocessor/run_case)
        ...
        # train_min = intensity_properties['percentile_00_5']
        # train_max = intensity_properties['percentile_99_5']
        # print(f"Training intensity range (percentile 0.5 ~ 99.5): {train_min:.2f} - {train_max:.2f}")
        # ####

        for idx in range(len(list_of_lists)):
            
            # #### added by hyenagatha02 (added in default_preprocessor/run_case)
            # # Load image(s) for scaling before preprocessing
            # image_files = list_of_lists[idx]
            # rw = plans_manager.image_reader_writer_class()
            # data, _ = rw.read_images(image_files)
            ...
            # print(f"Test intensity range (actual): {data_min:.2f} - {data_max:.2f}")
            ...
            # if data_percentile_00_5 < train_min or data_percentile_99_5 > train_max:
            #     print(f"Applying linear scaling for test dataset (Index {idx})")
            #     # Scale to 0-1 based on the test data's actual range
            #     data = (data - data_percentile_00_5) / (data_percentile_99_5 - data_percentile_00_5)
            #     # Scale back to the training data's intensity range
            #     data = data * (train_max - train_min) + train_min
            #     print(f"New Test intensity range: {np.min(data):.2f} - {np.max(data):.2f}")
            # ####
